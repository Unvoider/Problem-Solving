...
class AbsoluteMinHeap {
    ...
    push(data) {
        ...
        while(inPos !== 1 && this.compare(data, this.#heap[Math.trunc(parent)]) < 0) {
    ...
    pop() {
        ...
        while(child < heapLength){ // 자식이 존재하는 동안
            if(child + 1 < heapLength && this.compare(this.#heap[child], this.#heap[child + 1]) > 0)
                child += 1; // 왼쪽/오른쪽 자식 중 더 작은 값 고르기
            if(this.compare(last, this.#heap[child]) <= 0) break; // 부모 값이 자식 값보다 큰 동안
...
const q = new AbsoluteMinHeap(); // 절댓값 최소 힙
for(let i = 1; i <= n; i++) {
