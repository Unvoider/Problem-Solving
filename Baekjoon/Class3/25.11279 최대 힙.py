import sys
input = sys.stdin.readline
write = sys.stdout.write

class Heap:
    def __init__(self, cmp):
        self._cmp = cmp
        self._heap = [None] #root 인덱스 1
    def empty(self):
        return len(self._heap) == 1
    def push(self, data):
        in_pos = len(self._heap) #마지막 인덱스를 삽입 위치로
        parent = in_pos >> 1
        self._heap.append(None) #끝 자리 만들기
        #삽입 위치가 root가 아니고 자식 값이 부모 값보다 큰 동안
        while in_pos != 1 and self._cmp(data, self._heap[parent]) < 0:
            self._heap[in_pos] = self._heap[parent]; #부모를 자식으로
            in_pos >>= 1; #자식을 부모로
            parent = in_pos >> 1
        self._heap[in_pos] = data
    def pop(self):
        if self.empty(): raise IndexError("heap is empty")
        data = self._heap[1]
        last = self._heap.pop() #마지막 값을 root 자리에 올림
        if self.empty(): return data
        in_pos = 1
        child = 2
        heap_len = len(self._heap)
        while child < heap_len: #자식이 존재하는 동안
            if child + 1 < heap_len and self._cmp(self._heap[child], self._heap[child + 1]) > 0:
                child += 1 #왼쪽/오른쪽 자식 중 더 큰 값 고르기
            if self._cmp(last, self._heap[child]) <= 0: break #부모 값이 자식 값보다 작은 동안
            self._heap[in_pos] = self._heap[child] #자식을 부모로
            in_pos = child #부모를 자식으로
            child = in_pos << 1
        self._heap[in_pos] = last
        return data

def main():
    n = int(input())

    max_heap = Heap(lambda a, b: b - a)
    for _ in range(n):
        x = int(input())
        if x == 0:
            if max_heap.empty():
                write("0\n")
            else:
                write(f"{max_heap.pop()}\n")
        else:
            max_heap.push(x)

main()

''' 라이브러리 사용
import heapq
import sys
input = sys.stdin.readline
write = sys.stdout.write

max_heap = []

n = int(input())
for _ in range(n):
    x = int(input())
    if x == 0:
        if max_heap:
            write(f"{-heapq.heappop(max_heap)}\n")
        else:
            write("0\n")
    else:
        heapq.heappush(max_heap, -x)
'''