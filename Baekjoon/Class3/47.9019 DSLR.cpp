...
constexpr char A_OPERATIONS[] = { 'D', 'S', 'L', 'R' };
constexpr char B_OPERATIONS[] = { 'D', 'D', 'S', 'L', 'R' };

// 미리 DSLR 계산
int a_dslr[MAX_NUM + 1][4];
int b_dslr[MAX_NUM + 1][5];
void init_dslr() {
    for (int n = 0; n <= MAX_NUM; n++) {
        a_dslr[n][0] = (2 * n) % DIGIT_LIMIT;
        a_dslr[n][1] = n == 0 ? MAX_NUM : n - 1;
        a_dslr[n][2] = (n * 10) % DIGIT_LIMIT + n / (DIGIT_LIMIT / 10); // shift left + carry
        a_dslr[n][3] = (n / 10) + (n % 10) * (DIGIT_LIMIT / 10); // shift right + carry * 1000
        b_dslr[n][0] = n / 2;
        b_dslr[n][1] = (n + DIGIT_LIMIT) / 2;
        b_dslr[n][2] = n == MAX_NUM ? 0 : n + 1;
        b_dslr[n][3] = a_dslr[n][3]; // shift right + carry * 1000
        b_dslr[n][4] = a_dslr[n][2]; // shift left + carry
    }
}
...
void run_register() {
    ...
    while (true) {
        ...
        for (int _ = 0; _ < level_size; _++) { // 한 레벨씩
            ...
            // 정방향 탐색
            for (int i = 0; i < 4; i++) {
                int end = a_dslr[start][i];
                if (a_operations[end].second == NOT_VISITED) { // 연산자 저장
                    a_bfs.push(end);
                    a_operations[end] = { A_OPERATIONS[i], start };
                }
                if (b_operations[end].second != NOT_VISITED) { // 반대편과 만남
        ...
        for (int _ = 0; _ < level_size; _++) {
            ...
            // 역방향 탐색
            for (int i = (start % 2 == 0) ? 0 : 2; i < 5; i++) {
                int end = b_dslr[start][i];
                if (b_operations[end].second == NOT_VISITED) { // 연산자 저장
                    b_bfs.push(end);
                    b_operations[end] = { B_OPERATIONS[i], start };
                }
                if (a_operations[end].second != NOT_VISITED) { // 반대편과 만남
                    print_traces(end, a_operations, b_operations);
                    return;
                }
            }
